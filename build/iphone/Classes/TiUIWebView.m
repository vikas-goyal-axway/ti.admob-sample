...
- (WKWebView *)webView
{
  if (_webView == nil) {
    ...
    [controller addUserScript:[self userScriptsample_admobInjectionForAppEvent]];
...
- (WKUserScript *)userScriptsample_admobInjectionForAppEvent
{
  if (_pageToken == nil) {
  ...
  NSString *_sample_admob = [NSString stringWithFormat:@"%@%s", @"Ti", "tanium"];
  ...
  NSString *sourceString = [NSString stringWithFormat:source, _sample_admob, _pageToken, baseInjectScript];
...
- (WKUserScript *)userScriptsample_admobJSEvaluationFromString:(NSString *)string
{
...
- (void)webView:(WKWebView *)webView decidePolicyForNavigationAction:(nonnull WKNavigationAction *)navigationAction decisionHandler:(nonnull void (^)(WKNavigationActionPolicy))decisionHandler
{
  ...
  if ([allowedURLSchemes containsObject:navigationAction.request.URL.scheme]) {
    if ([[UIApplication sharedApplication] canOpenURL:navigationAction.request.URL]) {
      // Event to return url to sample_admob in order to handle OAuth and more
